...
class Http_:
    ...
    def add_route_(self, url_, func_, method_='GET', client_addr_=''):
        ...
        self.route_handlers_.append(
            Route_(route_parts_, method_, client_addr_, func_, route_args_)
        )
        ...
        try:
            ...
            if self.parse_first_line_():
                if self.parse_header_():
                    ...
                    route_handler_, route_args_ = self.http_.get_route_handler_(
                        self.path_, self.method_, self.addr_[0]
                    )
                    if route_handler_:
                        try:
                            ...
                            print(
                                'Http handler exception in route %s %s\r\n'
        ...
        try:
            ...
            if len(elements_) == 3:
                ...
                if len(elements_) > 1:
                    ...
                    self.params_ = {
                        unquote_plus_(k): unquote_plus_(v) for k, v in params_.items()
                    }
                return True
        except:
            pass
        ...
        if filepath_:
            ...
            if content_type_:
                if Http_.STATIC_CONTENT_CACHE_LEVEL > 0:
                    if (
                        Http_.STATIC_CONTENT_CACHE_LEVEL > 1
                        and 'if-modified-since' in self.headers_
                        and '__temp__' not in path_
                    ):
                        response_.write_response_not_modified_()
                    else:
                        headers_ = {
                            'Last-Modified': 'Fri, 1 Jan 2021 00:00:00 GMT',
                            'Cache-Control': 'max-age=315360000',
                            'Access-Control-Allow-Origin': '*',
                        }
                        response_.write_response_file_(
                            filepath_, content_type_, headers_
                        )
                else:
...
class Response_:
    ...
    def write_before_content_(
        self, code_, headers_, content_type_, charset_, content_length_
    ):
    ...
    def write_response_(self, code_, headers_, content_type_, charset_, content_):
        try:
            if content_:
                ...
                content_length_ = 0
            self.write_before_content_(
                code_, headers_, content_type_, charset_, content_length_
            )
            if content_:
        ...
        try:
            ...
            if size_ > 0:
                with open(filepath_, 'rb') as file_:
                    self.write_before_content_(
                        200, headers_, content_type_, None, size_
                    )
                    try:
                        buf_ = bytearray(256)
                        while size_ > 0:
    ...
    def write_response_OK_(
        self, headers_=None, content_type_=None, charset_=None, content_=None
    ):
